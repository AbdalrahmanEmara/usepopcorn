...
export default function App() {
  ...
  return (
    <>
      ...
      <Main>
        <Box>
          {isLoading && <Loader key={"Lod"} />}
          {!isLoading && !error && (
            <MoviesList movies={movies} onSelectMovie={handleSelectMovie} />
          )}
          {error && <ErrorMessage message={error} key={"Er"} />}
        </Box>
...
function WatchedMoviesList({ watched, onRemoveWatched }) {
  return (
    <ul className="list">
      {watched.map((movie) => (
        <WatchedMovie key={movie.imdbID} movie={movie} watched={watched} onRemoveWatched={onRemoveWatched} />
      ))}
    </ul>
  );
}
